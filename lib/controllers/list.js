  ...
  var criteria = Object.keys(q)
    .map(function(field) {
      return (!Array.isArray(q[field])) ?
        r.row(field).eq(q[field]) :
        q[field].map(function(value) { return r.row(field).eq(value); })
          .reduce(function(l, r) { return l.or(r); });
    })
    .reduce(function(left, right) { return left.or(right); });
  return query.filter(criteria);
};
...
List.prototype.fetch = function(req, res, context) {
  ...
  if (req.query.hasOwnProperty(searchParam)) {
  ...
  if (req.query.hasOwnProperty(sortParam) || !!this.resource.sort.default) {
